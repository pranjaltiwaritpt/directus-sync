...
import type { ClientConfigSchema, CollectionEnum, CollectionPreservableIdEnum, ConfigFileOptionsSchema, OptionsFields, OptionsSchema, RestConfigSchema } from './schema';
...
export type CollectionPreservableIdName = z.infer<typeof CollectionPreservableIdEnum>;
export type CollectionRecord<T> = Record<CollectionName, T>;
export type TransformDataFunction = <T = unknown>(data: T[], directusClient: Awaited<ReturnType<typeof MigrationClient.prototype.get>>) => T[] | Promise<T[]>;
export type TransformQueryFunction = <T = Query<DirectusBaseType>>(query: T, directusClient: Awaited<ReturnType<typeof MigrationClient.prototype.get>>) => T | Promise<T>;
export type TransformSnapshotFunction = (snapshot: Snapshot, directusClient: Awaited<ReturnType<typeof MigrationClient.prototype.get>>) => Snapshot | Promise<Snapshot>;
